...
const dbModel = models.QbSubject;
const modelName = 'QbSubject';
...
exports.index = (req, res) => {
  ...
  if (req.query._raw_s_id) {
    filterQuery['_raw.s_id'] = {
      [Op.eq]: `${req.query._raw_s_id}`,
    };
  }
...
exports.store = async (req, res) => {
  ...
  dbModel
    .create(req.body, {})
    .then((ret) => {
          if (req.body.assoc_grades) {
            ret.setAssoc_grades(req.body.assoc_grades);
          }
          res.send({
            status: 'success',
...
exports.show = (req, res) => {
  let includes = [];
  if (req.query.includeAssoc_grades) {
    includes.push({
      association: dbModel.associations['assoc_grades'],
      required: req.query.includeAssoc_grades == 'true' ? true : false,
    });
  }
