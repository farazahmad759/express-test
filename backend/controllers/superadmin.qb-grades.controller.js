...
const dbModel = models.QbGrade;
const modelName = 'QbGrade';
...
exports.index = (req, res) => {
  ...
  if (req.query._raw_class_level_id) {
    filterQuery['_raw.class_level_id'] = {
      [Op.eq]: `${req.query._raw_class_level_id}`,
    };
  }
  dbModel
    .findAndCountAll({
...
exports.store = async (req, res) => {
  ...
  dbModel
    .create(req.body, {})
    .then((ret) => {
      if (req.body.assoc_subjects) {
        ret.setAssoc_subjects(req.body.assoc_subjects);
      }
...
exports.show = (req, res) => {
  let includes = [];
  if (req.query.includeAssoc_subjects) {
    includes.push({
      association: dbModel.associations['assoc_subjects'],
      required: req.query.includeAssoc_subjects == 'true' ? true : false,
    });
  }
...
exports.update = async (req, res) => {
  dbModel
    ...
    .catch((err) => {
      res.send({
        status: 'success',
        message: modelName + ' could not update with ID = ' + req.params.id,
        error: err,
        count: null,
        data: req.body,
      });
